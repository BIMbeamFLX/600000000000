...
// Loopback-only test host. Throwaway Marmot secret is offline/e2e only; real people log in with Hangar + Alby or a Keycast bunker.
...
function home(url) {
  ...
  <header><b>600.wtf / GUILD WORKSPACE</b><p>LOCAL TEST HOST · throwaway Marmot secret is offline/e2e only · real login is Hangar + Alby or a Keycast bunker. HTTP stays on 127.0.0.1. Chat opens in White Noise.</p></header><nav>${links}</nav><main>${selected.map(tool => `<iframe title="${workspaceTools[tool].title}" data-tool="${tool}" sandbox="${tool === 'group-chat' ? 'allow-scripts allow-popups' : 'allow-scripts'}" src="/tool/${tool}"></iframe>`).join('')}</main>
